...
import { useQueryData } from 'Components/ApiQueries/WorkflowMasterApi'
...
function WorkflowList(props) {
    ...
    const [initiatorFinisherReferenceList, setinitiatorFinisherReferenceList] = useState([])
    ...
    const saveUpdateWorflow = () => {
        ...
        if (updateworkflowId != null) {
            axios.put(`http://192.168.0.166/api/crud/ulb-workflow-masters/${updateworkflowId}`, requestBody, header)
            ...
            axios.post('http://192.168.0.166/api/crud/ulb-workflow-masters', requestBody, header) // post workflow
                .then(function (response) {
                    setCustomLoader(false)
    ...
    const addEditWorflow = (worflowId = null) => {
        ...
        if (worflowId != null) {
            ...
            axios.get(`http://192.168.0.166/api/crud/ulb-workflow-masters/${worflowId}`, header)// for live api
                .then(function (response) {
                    ...
                    const candidateIdJsonArray = response.data[0].candidate_id.map((candidateId) => {
                        return { id: candidateId }
                    })
                    const candidateNameJsonArray = response.data[0].candidate_name.map((candidateName) => {
        ...
        axios.get('http://192.168.0.166/api/all-workflows', header) // all workflow list
            .then(function (response) {
        ...
        axios.get('http://192.168.0.166/api/get-all-ulb', header) //all ulb list
            .then(function (response) {
        ...
        axios.get('http://192.168.0.166/api/crud/module-masters/create', header) // all module list
            .then(function (response) {
        ...
        axios.get('http://192.168.0.166/api/get-all-users', header) // all user list
            .then(function (response) {
                //filtering the incoming data for multiselect
                const candidateList = response.data.filter(data => {
        ...
        ,
        {
            ...
            Cell: ({ cell }) => (
                <div className="flex gap-4">
                    ...
                    <button onClick={() => passWorkflowData(cell.row.values.id)} className='flex-initial bg-sky-200 px-3 py-1 rounded-lg shadow-lg hover:shadow-xl hover:bg-sky-800 hover:text-white text-black items-center flex'>
    ...
    const { isLoading, data, isError, error, refetch } = useQuery("workflow-list", () => {
        return axios.get("http://192.168.0.166/api/crud/ulb-workflow-masters/create", header);
    }, {
        onSuccess
    });
    ...
    function onSelectCandidate(selectedList, selectedItem) {
        ...
        setinitiatorFinisherReferenceList([...initiatorFinisherReferenceList, selectedItem])
    ...
    return (
        <>
            ...
            {isError && <ErrorPage/>}
            {!isLoading && (!isError && <ListTable columns={COLUMNS} dataList={data?.data}><button onClick={() => addEditWorflow()} className="float-right bg-green-500 px-3 py-1 rounded-sm shadow-lg hover:shadow-xl hover:bg-green-800 hover:text-white text-white flex items-center"><CgPlayListAdd />Add</button></ListTable>)}
                    ...
                    <div className={`p-10 px-10 pb-32 ${updateworkflowId == null ? 'bg-green-400' : 'bg-gray-400'}   text-white`} style={{ 'width': '60vw' }}>
                        ...
                        <form className=" md:w-full md:grid md:grid-cols-2 md:gap-x-8" onChange={handleChange} onSubmit={formik.handleSubmit}>
                            <div className="mb-4">
                                ...
                                <span className="text-red-600 absolute">{formik.touched.workflowName && formik.errors.workflowName ? formik.errors.workflowName : null}</span>
                            </div>
                            <div className="mb-4">
                                <label className={'block text-sm font-medium'}>Select ULB</label>
                                <select {...formik.getFieldProps('ulb')} className="h-10 bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 shadow-lg cursor-pointer" >
                                ...
                                <span className="text-red-600 absolute">{formik.touched.ulb && formik.errors.ulb ? formik.errors.ulb : null}</span>
                            </div>
                            <div className="mb-4">
                                <label className={'block text-sm font-medium'}>Select Module</label>
                                <select {...formik.getFieldProps('module')} className="h-10 bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 shadow-lg cursor-pointer" >
                                    <option value="_">== Select ==</option>
                                ...
                                <span className="text-red-600 absolute">{formik.touched.module && formik.errors.module ? formik.errors.module : null}</span>
                            </div>
                            <div className="mb-4">
                            ...
                            <div className="mb-4">
                                ...
                                <select {...formik.getFieldProps('initiator')} className="h-10 bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 shadow-lg cursor-pointer" >
                                    <option value="_">== Select ==</option>
                            ...
                            <div className="mb-4">
                                ...
                                <select {...formik.getFieldProps('finisher')} className="h-10 bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 shadow-lg cursor-pointer" >
                                    <option value="_">== Select ==</option>
                            ...
                            <div className="text-left col-span-2 relative top-20">
                                ...
                                <div className="col-span-2">{updateworkflowId == null && (!customLoader && <button onClick={handleSubmit} type="button" className="text-white bg-green-700 border-2 border-white focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm w-full sm:w-auto px-5 py-2.5 text-center hover:bg-green-400 hover:shadow-2xl">Submit</button>)}
                                    {updateworkflowId != null && (!customLoader && <button onClick={handleSubmit} type="button" className="text-white bg-blue-700 border-2 border-white focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm w-full sm:w-auto px-5 py-2.5 text-center hover:bg-blue-500 hover:shadow-2xl">Update</button>)}
                                    {customLoader && <Loader />}</div>
                            </div>
                        </form>
                    </div>

                    <img src={''} className=" w-40 absolute bottom-0 right-10" alt="image" />
                </div>
            </Modal>
            ...
            >
                <div className="relative bg-white rounded-lg shadow-2xl border-2 border-gray-50 rounded" >
                    <div className="p-10 px-10 md:w-96 bg-red-500  text-center">
                        <h1 className="text-white text-lg">Do you want to delete <b><i>{delteworkflowName}</i></b> workflow ?</h1>
