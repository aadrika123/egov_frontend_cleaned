...
function UlbWorkFlowMaping() {
    ...
    const { isLoading, data, isError, error } = useQuery("UlbWorkFlowMaping-Query", () => {
        return axios.get('http://192.168.0.166/api/get-all-ulb');
    });

    //Fetch Workflow List and ID
    const { isLoading: isLoadingWorkflowList, data: workflowListData, refetch: refetchWorkFlowList, isError: isErrorWorkflowList, error: errorWorkflowList } = useQuery("UlbWorkFlowListDatas", () => {
    ...
    useEffect(() => {
        ...
        axios.get(`http://192.168.0.166/api/workflow/workflow-roles?ulbID=${roleUlb}&workflowID=${worlflowId}`, header)
                ...
                <div className="border shadow-lg">
                    <div className="p-2 shadow-md bg-gradient-to-r from-indigo-300 via-purple-300 to-pink-300">
                        <div className='grid grid-cols-2 '>
                            <div className='col-span-1'> <span className='text-xl font-semibold'>ULB Workflow Roles Mapping</span></div>
                ...
                <div className='py-3'>
                    <div className='grid grid-cols-4 mb-2'>
                        {roles &&
                            ...
                            roles?.data.data.map((e, i) => (
                                <div kay={i}></div>,
                                <div className='col-span-1 '>
                                    <div htmlFor="roles" style={{ backgroundColor: `${e.permission_status == true ? '#2dd240' : '#d9cac8'}` }} className={`flex border shadow-yellow-200 shadow-sm px-2 py-1 mt-2  mx-2 rounded-sm cursor-pointer hover:animate-pulse border-1 border-${e.permission_status == true ? 'black' : 'gray-400'}`}>
                                        <div className='grid grid-cols-12'>
                                            <div className='col-span-1'>
                                                <input
                                                    ...
                                                    onChange={()=>handleOnChange(e.ulb_workflow_id, e.role_id, e.permission_status)}
                                                    // value={e.role_id}
                                                    value={e.role_id}
                                                />
                                            </div>
                                            ...
                                            <div className='col-span-1'>
                                                <p className='font-semibold mt-1'>{e.status == true && <TbChecks color='blue' />}</p>
                                            </div>
                                        </div>
                                    </div>
                                </div>
