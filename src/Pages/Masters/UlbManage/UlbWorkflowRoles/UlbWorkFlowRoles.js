...
function UlbWorkFlowRoles() {
    ...
    const { isLoading, data, isError, error } = useQuery("UlbWorkFlowRolesJSThis-Query", () => {
        return axios.get('http://192.168.0.166/api/get-all-ulb');
    });

    //Fetch Workflow List and ID
    const { isLoading: isLoadingWorkflowList, data: workflowListData, refetch: refetchWorkFlowList, isError: isErrorWorkflowList, error: errorWorkflowList } = useQuery("UlbWorkFlowListDatasthisNItdfjgoisduw", () => {
        return axios.get(`http://192.168.0.166/api/admin/workflows/${roleUlb}`, header);
    });
    ...
    useEffect(() => {
        ...
        axios.get(`http://192.168.0.166/api/ulb/workflow/member?ulbID=${roleUlb}&workflowsID=${worlflowId}`, header)
            .then(function (response) {
        ...
        <>
            <div className='border shadow-xl'>
                <div className="border shadow-lg">
                    <div className="p-2 shadow-md bg-gradient-to-r from-indigo-300 via-purple-300 to-pink-300">
                        <div className='grid grid-cols-2 '>
                            <div className='col-span-1'> <span className='text-xl font-semibold'>Manage ULB Workflow Roles</span></div>
                        </div>
                    </div>
                </div>
                        ...
                        >
                            ...
                            {!isLoadingWorkflowList ?
                                workflowListData?.data.data.map((worflow) => (
                                    <option value={worflow.workflow_id}>{worflow.workflow_name}</option>
                                )) : ''
                            }
                        </select>
                    </div>
                        ...
                        {roles?.data.status &&
                            <div className='flex'>
                                <div>
                                    ...
                                    <select onChange={(e) => setInitiatorId(e.target.value)} className='border border-black h-8 w-48 mx-3 px-2 rounded-bl-lg font-semibold'>
                                        <option  > -- Choose Initiator -- </option>
                                        {
                                            roles?.data.data.rolls?.map((e, i) => (
                                                <td>{i}</td>,
                                ...
                                <div>
                                    ...
                                    <select onChange={(e) => setFinisherId(e.target.value)} className='border border-black h-8 w-48 mx-3 px-2 rounded-br-lg font-semibold'>
                                        <option value="" > -- Choose Finisher -- </option>
                                        {
                                            // roles?.data?.data.map((e) => (
                                            roles?.data.data.rolls?.filter(item => item.rolle_id != initiatorId).map((e) => (
                                                <td>{e.id}</td>,
                                                <option value={e.rolle_id}>{e.role_name}</option>
                                            ))}
                                    </select>
                ...
                <div>
                        ...
                        <table className='w-full'>
                            ...
                            </tr>
                            {
                                roles?.data.data.rolls?.map((e, i) => (
                                    ...
                                    <tr className={`text-center h-8 bg-green-${e.show_full_list && 100}`}>
                                        <td className='font-medium'>{e.role_name}({e.rolle_id})</td>
                                        {/* <input onChange={(e) => setRoleID(e.target.value)} type="hidden" name="RoleID" value={e.rolle_id} /> */}
                                        <td>
                                            <select onChange={(a) => { setsForwardTo(a.target.value); }} name="forwodID" className='border border-black w-48'>
                                        ...
                                        <td>
                                            <select  onChange={(a) => { setsBackwardTo(a.target.value); setRoleID(e.rolle_id); Aactions(); }} name="backwodID" className='border border-black w-48'>
                                                <option value=""> -- Select -- </option>
                                                {
                                                    roles?.data.data.rolls?.filter(item => item.rolle_id != finisherId).map((e) => (                                                        
                                                        <option value={e.rolle_id}>{e.role_name}</option>
                                                    ))}
                                            </select>
                                        </td>
                                        <td>
                                            <input
                                                ...
                                                onChange={(e) => handleFullAccessCheckBox(e.target.value, e.target.checked)}
                                                value={e.rolle_id}
                                            />
                                            <span className={`font-semibold text-${e.show_full_list ? 'green-600' : 'black'}`}> Full Access</span>
                                        </td>
                                    </tr>
                                ))
                            }
                    ...
                    <div className='flex justify-center my-5'>
                        <button onClick={handleAssignbtn} className='px-5 py-1 mx-5 bg-blue-600 hover:bg-blue-700 hover:rounded-md font-semibold text-white rounded-sm shadow-sm' type="submit">Assign</button>
                    </div>
                </div>
            </div>
        </>
