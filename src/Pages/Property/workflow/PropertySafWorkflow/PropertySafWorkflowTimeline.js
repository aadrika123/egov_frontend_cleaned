...
function PropertySafWorkflowTimeline(props) {
    ...
    const { api_postEscalateStatus, api_postComment, api_postApplicationToLevel, api_fetchRoleDetail, api_approveRejectForm, api_backToCitizen } = ProjectApiList()
    ...
    const sendApplicationToLevel = (e) => {
        ...
        axios.post(`${api_postApplicationToLevel}`, requestBody, header)
            .then(function (response) {
                console.log("application forwarded", response)
                props.showTabFun(false);    //hiding tabs
                { (e.target.id == 'btn_forward') && props.toast('Application is forwarded successfully', "escalated") }
                { (e.target.id == 'btn_back') && props.toast('Application send backward successfully', "de-escalated") }
    ...
    const escalateAction = (status) => {
        ...
        console.log('---------------------------------escalate body....', props?.applicationData?.data?.id)
        // return
        axios.post(`${api_postEscalateStatus}`, requestBody, header)
            .then(function (response) {
                { (escalateStatus == 1) && props.toast('Application Escalated Successfully!', "escalated") }
                { (escalateStatus == 0) && props.toast('Application De-Escalated Successfully!', "de-escalated") }
            })
            .catch(function (error) {
                props.toast('Oops! Something went wrong', "error")
            })
    ...
    const sendBackToCitizen = (e) => {
        ...
        axios.post(`${api_backToCitizen}`, requestBody, header)
            .then(function (response) {
                console.log("application forwarded ", response)
                props.showTabFun(false);    //hiding tabs
                { (e.target.id == 'btn_backToCitizen') && props.toast('Application is forwarded to BTC successfully', "escalated") }
            })
            .catch(function (error) {
                props.toast('Oops! Something went wrong', "error", error)
            })
    ...
    const approveRejectApplication = (e) => {
        ...
        axios.post(`${api_approveRejectForm}`, requestBody, header)
            .then(function (response) {
                console.log("application status for approve a reject", response)
                props.showTabFun(false);    //hiding tabs
                { (e.target.value == '1') && props.toast('Application is Approved ', "escalated") }
                { (e.target.value == '0') && props.toast('Application is Rejected', "de-escalated") }
            })
            .catch(function (error) {
                props.toast('Oops! Something went wrong', "error", error)
            })
    ...
    console.log('current role id of user .....', props?.applicationData?.data?.current_role)
    console.log('workFlow Id on workflowtimelinfe .....', props?.applicationData?.data?.workflow_id)

    return (
        <>
            <div className={` grid grid-cols-12 shadow-lg border-2 border-gray-200 relative overflow-hidden h-auto`}>
                <div className={'bg-gray-200 col-span-12 md:col-span-3 h-auto'}>
                    ...
                    <div className='px-2 mt-4 h-auto'>
                        ...
                        <div className='h-28'><TextArea commentFun={commentFun} bgColor="bg-gray-100" value={commentText} /></div>
                        <div className="flex mt-2">
                            ...
                            <div className='flex-1'><button onClick={sendIndependentComment} style={{ 'fontSize': '10px' }} className='bg-green-500 text-white rounded-sm px-2 hover:shadow-lg py-1'>Send Comment</button></div>

                        </div>
                        <div><CheckBoxInput is_escalate={props?.applicationData?.is_escalate} fun={swithEscalateStatus} /></div>
                        <div className='flex'>
                            <div className='flex-1 '>
                                <button type='button' id='btn_back' value={roleDetails.backward_role_id} className='w-full bg-sky-500 border-2  shadow-lg text-white text-sm font-semibold rounded-lg  focus:outline-none focus:shadow-outline  hover:shadow-xs p-3 py-2 hover:bg-sky-600 hover:text-white my-4 text-center' onClick={sendApplicationToLevel}>back</button>
                            </div>
                            {props?.applicationData?.data?.current_role != 10 &&
                                <div className='flex-1 '>
                                    <button type='button' id='btn_forward' value={roleDetails.forward_role_id} className='w-full bg-sky-500 border-2  shadow-lg text-white text-sm font-semibold rounded-lg  focus:outline-none focus:shadow-outline  hover:shadow-xs p-3 py-2 hover:bg-sky-600 hover:text-white my-4 text-center' onClick={sendApplicationToLevel}>forward</button>
                                </div>
                            }
                        </div>
                        {props?.applicationData?.data?.current_role != 5 &&  props?.applicationData?.data?.current_role != 11 &&
                            <div className='flex-1 '>
                                <button type='button' id='btn_backToCitizen' value='' className='w-full bg-sky-500 border-2  shadow-lg text-white text-sm font-semibold rounded-lg  focus:outline-none focus:shadow-outline  hover:shadow-xs p-3 py-2 hover:bg-sky-600 hover:text-white my-4 text-center' onClick={sendBackToCitizen}>back to citizen</button>
                            </div>
                        }
                        ...
                        {props?.applicationData?.data?.current_role == 10 &&
                            <div className={`flex`}>
                                <div className='flex-1 '>
                                    <button type='button' id='btn_approve' value={1} className="block w-full bg-green-500 border-2  shadow-lg text-white text-sm font-semibold rounded-lg  focus:outline-none focus:shadow-outline  hover:shadow-xs p-3 py-2 hover:bg-green-600 hover:text-white my-4" onClick={approveRejectApplication}>Approved</button>

                                </div>
                                <div className='flex-1 '>
                                    <button type='button' id='btn_reject' value={0} className="block w-full bg-red-500 border-2  shadow-lg text-white text-sm font-semibold rounded-lg  focus:outline-none focus:shadow-outline  hover:shadow-xs p-3 py-2 hover:bg-red-600 hover:text-white my-4" onClick={approveRejectApplication}>Reject</button>
                                </div>
