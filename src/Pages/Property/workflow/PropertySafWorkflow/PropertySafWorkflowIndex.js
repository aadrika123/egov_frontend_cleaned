...
function Mailbox() {
    ...
    const fetchDetailsById = () => {
        ...
        axios.post(`${api_getWorkflowCandidates}`, { workflowId: 4 }, header)
    ...
    const { isLoading, data, isError, error } = useQuery("safCandidateList", () => {
        ...
        <>
            ...
            <div className="grid grid-cols-12 rounded-lg mt-0 -ml-10 shadow-xl broder-2 border-sky-200 bg-gray-200">
                <div className='col-span-12 sm:col-span-2 '>
                    ...
                    {!isLoading && (!isError && <MailboxSidebar candidateListStatus={true} workflowCandidates={workflowCandidate} tabs={tabs} fun={tabSwitch} />)}
                ...
                </div> {/** MailboxSidebar - common mailbox sidebar component */}
                {tabIndex == 0 && <div className='col-span-12 sm:col-span-10 shadow-lg bg-white overflow-y-scroll' style={{ 'height': '90vh' }}><PropertySafListBox workflowCandidates={workflowCandidate} boxType="inbox" boxUrl="http://192.168.0.166/api/property/saf/inbox" /></div>}
                {tabIndex == 1 && <div className='col-span-12 sm:col-span-10 shadow-lg bg-white overflow-y-scroll' style={{ 'height': '90vh' }}><PropertySafListBox boxType="outbox" boxUrl="http://192.168.0.166/api/saf-out" /></div>}
                {tabIndex == 2 && <div className='col-span-12 sm:col-span-10 shadow-lg bg-white overflow-y-scroll' style={{ 'height': '90vh' }}><Search /></div>}
                {tabIndex == 3 && <div className='col-span-12 sm:col-span-10 shadow-lg bg-white overflow-y-scroll' style={{ 'height': '90vh' }}><PropertySafListBox workflowCandidates={workflowCandidate} boxType="specialbox" boxUrl="http://192.168.0.166/api/property/saf/escalate/inbox" /></div>}
