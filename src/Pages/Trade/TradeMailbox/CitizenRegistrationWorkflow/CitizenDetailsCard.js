...
function CitizenDetailsCard(props) {
    ...
    var { isLoading, data, isError, error, refetch } = useQuery("Citizen-by-id", () => {
        return axios.get(`http://192.168.0.166/api/get-citizen-by-id/${props?.id}`, header);
    }, {
        enabled: true,
        onSuccess
    });
                    ...
                    <div className="md:flex no-wrap md:-mx-2 ">
                        <div className="w-full md:w-3/12 md:mx-2 shadow-xl">
                            <div className="bg-white p-3 border-t-4 border-green-400">
                                ...
                                <h1 className="text-gray-900 font-bold text-xl leading-8 my-1 text-center">{cardData?.name}</h1>
                        ...
                        <div className="w-full md:w-9/12 mx-2 h-auto">
                            <div className="bg-white p-3 shadow-xl rounded-sm pb-40">
                                <div className="flex items-center pl-4 space-x-2 font-semibold text-gray-900 leading-8">
                                    ...
                                    <span className="tracking-wide animate__animated animate__rubberBand">About Citizen</span>
                                </div>
                                <div className="text-gray-700">
                                    <div className="grid md:grid-cols-2 text-sm">
                                        <div className="flex">
                                            <div className="flex-initial px-4 py-2 font-semibold">Citizen Name :</div>
                                            <div className="flex-initial px-4 py-2">{cardData?.user_name}</div>
                                        </div>
                                        <div className="flex wrap">
                                            <div className="flex-initial px-4 py-2 font-semibold">Email :</div>
                                            <div className="flex-initial px-4 py-2">{cardData?.email}</div>
                                        </div>
                                        <div className="flex">
                                            <div className="flex-initial px-4 py-2 font-semibold">Mobile :</div>
                                            <div className="flex-initial px-4 py-2">{cardData?.mobile}</div>
                                        </div>
                                        <div className="flex">
                                            <div className="flex-initial px-4 py-2 font-semibold">ULB :</div>
                                            <div className="flex-initial px-4 py-2">{cardData?.ulb_name}</div>
                                        </div>
