...
function CitizenWorkflowTimeline(props) {
    ...
    const citizenActionProceed = () => {
        ...
        if (actionName == 'Approve') {
            axios.post(`http://192.168.0.166/api/edit-citizen-by-id/${props?.id}`, { isApproved: 1 }, header)
        ...
        if (actionName == 'Reject') {
            axios.post(`http://192.168.0.166/api/edit-citizen-by-id/${props?.id}`, { isApproved: 0 }, header)
                .then(function (response) {
    ...
    return (
        <>
            <ToastContainer position="top-right"
                autoClose={2000} />
            {
                buttonStatus && <div className="flex gap-4">
                    <button onClick={() => approveAction('Approve')} className='flex-initial bg-green-200 px-3 py-1 rounded-lg shadow-lg hover:shadow-xl hover:bg-green-800 hover:text-white text-black flex items-center'>
                        ...
                        <FiEdit className="inline" />&nbsp;Approved
                    </button>
                    <button onClick={() => approveAction('Reject')} className='flex-initial bg-red-200 px-3 py-1 rounded-lg shadow-lg hover:shadow-xl hover:bg-red-800 hover:text-white text-black items-center flex'>
                        <MdDeleteForever className="inline" />Reject
                    </button>
            ...
            >
                <div className="relative bg-white rounded-lg shadow-2xl border-2 border-gray-50 rounded" >
                    <div className={`p-10 px-10 md:w-96 bg-white  text-center border-2 ${(actionName == 'Approve')?'border-green-400':'border-red-400'}`}>

                        <h1 className="text-black text-lg mb-2">Do you want to <b><i>{actionName}</i></b> citizen ?</h1>
                        {(actionName == 'Approve') && <img src={approve} className="w-2/3 mx-auto" alt="deleteImage" />}
                        {(actionName == 'Reject') && <img src={deleteImage} className="w-2/3 mx-auto" alt="deleteImage" />}
                        <div>
                            <h1 className='text-sm text-left'>Write comment</h1>
                            <TextArea commentFun={commentFun} bgColor="bg-white" value={commentText} />
                        </div>

                        <button onClick={citizenActionProceed} className={`${(actionName == 'Approve') ? 'text-green-600 border-green-600 hover:bg-green-800' : 'text-red-600 border-red-600 hover:bg-red-800'} bg-white border border-white shadow-lg rounded-lg px-2 py-1 text-white  mt-5 text-lg  hover:text-white hover:shadow-3xl md:mr-3 font-semibold`}><MdDeleteForever className="inline" />{actionName}</button>
