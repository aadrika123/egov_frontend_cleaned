...
function OwnerDetails(props) {
    ...
    let ownerDetails = [];  //empty array for storing restructured values
    props.values.licenseData?.ownerDtl.forEach((item) => {

        let temp = { "businessOwnerName": item.owner_name, "guardianName": item.guardian_name, "mobileNo": item.mobile, "email": item.emailid, "documentId": item.document_id }

        ownerDetails.push(temp);
    })

    console.log("ownerDtl converted", ownerDetails);
    const initialValues = {
    ...
    const handleMultipleSubmit = () => {
        ...
        collectFormDataFun('ownerDetails', ownerDetails, 0) //sending BasicDetails data to parent to store all form data at one container
    ...
    // }
    let divStyle = "bg-gray-200  border border-gray-800 mr-4 mr-2";
    let lblStyle = "text-xs font-semibold";
    let containerStyle = " col-span-1";
    return (
        <>
