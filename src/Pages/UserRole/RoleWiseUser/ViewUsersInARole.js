...
function ViewUsersInARole(props) {
  ...
  const { isLoading, data, isError, error } = useQuery("ViewUsersInARole-querty", () => {
    try {
      return axios.get(`http://192.168.0.166/api/master/users/user-by-role-id/${props.btnId}`, header);
    } catch (err) {
      ...
      <div className={`mx-1 border shadow-lg bg-green-50`}>
        <div className={`p-2 bg-green-400 shadow-lg`}>
          <div className='grid grid-cols-2 '>
            <div className='col-span-1 flex'><p className='text-xl font-semibold'>{props.roleName} </p> <p className='text-base mt-1 ml-2'> Role Given to Users</p></div>
        ...
        <div className='pb-5'>
          {!isLoading && <UserPermissionDataTable searchText="Roles" columns={columns} data={data?.data.data} />}       </div>
      </div>
    </>
  )
}
