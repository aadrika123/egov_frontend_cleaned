...
function DeleteUderRole(props) {
  ...
  const deleteBtn = () => {
    ...
    axios
      .delete(`http://192.168.0.166/api/delete-role/${props?.data.id}`, header)
      .then(function (res) {
  ...
  return (
    <>
      ...
      <button
        onClick={deleteBtn}
        className="bg-red-400 hover:bg-red-500 mr-2 px-5 py-2 border border-black font-medium text-xl rounded-sm"
      >
        Delete
      </button>
      <button
        onClick={closeDialoge}
        className="bg-blue-400 hover:bg-blue-500 ml-2 px-5 py-2 border border-black font-medium text-xl rounded-sm"
      >
        Cancel
      </button>
    </>
