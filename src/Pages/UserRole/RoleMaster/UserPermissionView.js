...
function UserPermissionView(props) {
    ...
    useEffect(() => {
        axios.get(`http://192.168.0.166/api/get-role-user/${props.btnId}`, header)
    ...
    const { isLoading, data: AllRoles, isError, error } = useQuery("get-all-roles-UserPermissionView", () => {
            ...
            <div className="border shadow-lg bg-green-50">
                <div className="p-2 bg-green-400 shadow-lg">
                    <div className='grid grid-cols-2 '>
                        <div className='col-span-1 text-xl font-semibold'>Manage User Permission</div>
                ...
                <div className='p-5 '>
                    ...
                    <div className='grid grid-cols-12'>
                        <div className='col-span-6 border-2 mx-5 shadow-lg'>
                            <h1 className='font-semibold text-center bg-blue-400 py-1'>Given Roles</h1>
                            {
                                fetchedData?.role_name.map((e, i) => (
                                    <div kay={e.id}></div>,

                                    <div className='space-y-1 mx-2 text-gray-700 border bg-amber-100 pl-2 my-2 py-2'>
                                        <div className='grid grid-cols-12 px-2'>
                                            <div className='col-span-6'>
                                                <div><span className='font-semibold'>{fetchedData?.role_name[i]} </span> </div>
                                            </div>
                                            <div className='col-span-4'>
                                                <select className='shadow-xl border' name="permissin" id="">
                                                    <option value="0">{fetchedData?.can_modify[i] == "true" ? "Read" : "Write"}</option>
                                                    <option value="0">{fetchedData?.can_modify[i] == "false" ? "Read" : "Write"}</option>
                                            ...
                                            <div className='col-span-1 cursor-pointer hover:bg-sky-200 rounded-full mt-1'>
                                                <p><BsFillCheckCircleFill className='mt-0.5' onClick={() => handleSaveIcon(e.id)} fill='green' size={16} /></p>
                                            </div>
                                            <div className='col-span-1 cursor-pointer hover:bg-red-200 rounded-full mt-1'>
                                                <p><TiDelete onClick={() => handleDeleteBtn(e.id)} fill='red' size={20} /></p>
                                            </div>
                                        </div>
                                    </div>
                                ))
                        ...
                        <div className='col-span-6 border-2 mx-5 shadow-lg'>
                            <h1 className='font-semibold text-center bg-blue-400 py-1'>Avalible Roles</h1>
                            ...
                            <div className='my-5 flex justify-center'>
                                <button onClick={handleAssignBtn} className='px-4 py-1 bg-blue-500 hover:bg-blue-600 shadow-lg'>Assign Roles</button>
                            </div>
                        </div>
                    </div>
                </div>
