...
function UserWardPage(props) {
    ...
    const update = (ward, status) => {
        ...
        axios({
            method: "post",
            url: `http://192.168.0.166/api/ward/masters/ward-user`,
            data: {
                "userID": props.btnId,
                "ulbWardID": ward,                
                "status": status
    ...
    const { isLoading, data, refetch, isError, error } = useQuery("UserWardPage-fetchAPi", () => {
        return axios.get(`http://192.168.0.166/api/ward/masters/ward-user/${props.btnId}`, header);
    });
    if (!isLoading) { }
                ...
                <div className='mb-12 border bg-gray-100'>
                    <div className='grid grid-cols-12'>
                        <div className='col-start-2 h-10 w-40 py-2 px-4 my-3 bg-blue-300 flex justify-self-start border shadow-md'>
                            <input type="checkbox" className='h-5 w-5' name="" id="" /> <p className='ml-2 font-semibold'> Select All</p>
                        </div>
                        <div className='col-start-10'>
                            {/* <button className='h-10 w-40 py-2 px-4 my-3 bg-blue-300 text-center font-semibold border shadow-md'>Save</button> */}
                            <button className='h-10 w-40 py-2 px-4 my-3 bg-green-400 hover:bg-green-500 hover:rounded-md text-center font-semibold border-black shadow-md'>Save</button>
                        </div>
                    </div>
                        ...
                        {data &&
                            data?.data.data.map((e, i) => (
                                ...
                                <div className='col-span-1 '>
                                    <div className={`flex border shadow-2xl px-2 py-1 mt-2 bg-${e.status == true ? 'yellow-200' : 'white'} mx-2 cursor-pointer hover:animate-pulse border-1 border-${e.status == true ? 'black' : 'gray-500'}`}>
